...
export const stackIcons: Record<string, { path: string; hex: string; viewBox?: string }> = {
  ...
  // OpenAI's mark has no fixed brand color — it's designed to adapt to light/dark
  // backgrounds, so white here isn't a modification, just its normal presentation.
  Codex: {
...
// AWS's guidelines require the logo to stay unmodified — official white/orange
// variant for dark backgrounds, extracted as-is, never recolored on hover.
export const fixedColorIcons: Record<string, { viewBox: string; paths: { d: string; fill: string }[] }> = {
  AWS: {
    viewBox: "0 0 256 256",
    paths: [
      {
...
// Some official brand colors (e.g. Angular's near-black rebrand, OpenJDK's pure black)
// are invisible on our dark background — fall back to the default text color for those.
export function iconColorFor(hex: string): string | undefined {
  const r = parseInt(hex.slice(0, 2), 16);
  const g = parseInt(hex.slice(2, 4), 16);
  const b = parseInt(hex.slice(4, 6), 16);
  const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
  return luminance < 0.15 ? undefined : `#${hex}`;
}

// Angular's official flat brand color (#0F0F11) is unreadable on a dark background.
// Their gradient identity (red → pink → purple) isn't published anywhere we can verify
// exactly, so this is an approximation, not a pixel-perfect match to the official asset.
export const stackGradients: Record<string, [string, string, string]> = {
  Angular: ["#E40035", "#FA1147", "#C822FF"]
};
